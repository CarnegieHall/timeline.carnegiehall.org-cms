...
class PageResource extends JsonResource
{
  ...
  {
    $lite = str_contains($request->route()->uri, 'v2') && !str_contains($request->route()->uri, '{page}');
    $blocks = [];
    ...
    if (!$lite) {
      ...
      foreach ($blocks as $block) {
        ...
        if ($block->type == 'all-authors') {
          $authors = Author::ordered()->get();
          $block['authors'] = AuthorResource::collection($authors);
          unset($block->content);
          unset($block->editor_name);
          unset($block->medias);
          unset($block->children);
        ...
        foreach ($block->children as $child) {
          ...
          if ($child->type == 'media-collection-item') {
            ...
            // Use Twill's imageAsArray method for proper image handling (matching frontend pattern)
            $child->image = $child->imageAsArray('image', 'desktop');
    ...
    return [
      ...
      'title' => $this->title,
      'slug' => $this->slug,
      'nested_slug' => $this->getNestedSlug(),
