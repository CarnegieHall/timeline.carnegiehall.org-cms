    ...
    <div id="test" class="listing">
                            ...
                            @foreach ($hiddenFilters as $filter)
                                @php
                                    $options = $filter
                                        ...
                                        ->map(function ($label, $value) use ($filter) {
                                            if (
                                                $value === \A17\Twill\Services\Listings\Filters\BasicFilter::OPTION_ALL
                                            ) {
                                                $label = twillTrans('twill::lang.listing.filters.all-label', [
                                    ...
                                    $currentValue =
                                        $requestFilter[$filter->getQueryString()] ?? $filter->getDefaultValue();

                                    $selectedIndex = array_search($currentValue, array_column($options, 'value'), true);
                                @endphp
                                <a17-vselect name="{{ $filter->getQueryString() }}" :options="{{ json_encode($options) }}"
                                    ...
                                    ref="filterDropdown[{{ $loop->index }}]"></a17-vselect>
                            @endforeach
                        </div>
                    @else
                    ...
                    @if (isset($additionalTableActions) && count($additionalTableActions))
                        <div slot="additional-actions">
                            @foreach ($additionalTableActions as $additionalTableAction)
                                <a17-button variant="{{ $additionalTableAction['variant'] ?? 'primary' }}"
        ...
        @if ($create || (isset($tableData[0]['editInModal']) && !empty($tableData[0]['editInModal'])))
            <a17-modal-create ref="editionModal" form-create="{{ $storeUrl }}" v-on:reload="reloadDatas"
                ...
                <a17-langmanager :control-publication="{{ json_encode($controlLanguagesPublication) }}"></a17-langmanager>
        ...
        <a17-dialog ref="warningDeleteRow" modal-title="{{ twillTrans('twill::lang.listing.dialogs.delete.title') }}"
            ...
            <p class="modal--tiny-title">
                <strong>{{ twillTrans('twill::lang.listing.dialogs.delete.move-to-trash') }}</strong>
            </p>
