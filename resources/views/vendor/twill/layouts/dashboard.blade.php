...
@section('primaryNavigation')
    @if (config('twill.enabled.search', false))
        <div class="dashboardSearch" id="searchApp" v-cloak>
            <a17-search endpoint="{{ route(config('twill.dashboard.search_endpoint')) }}" type="dashboard"
                placeholder="{{ twillTrans('twill::lang.dashboard.search-placeholder') }}"></a17-search>
        </div>
    @endif
@stop
            ...
            @if (($facts ?? false) || !$drafts->isEmpty())
                <div class="wrapper wrapper--reverse">
                    <aside class="col col--aside">
                        ...
                        @if (!$drafts->isEmpty())
                            <a17-feed
                                :entities="{{ json_encode($drafts ?? []) }}">{{ twillTrans('twill::lang.dashboard.my-drafts') }}</a17-feed>
                        @endif
                    </aside>
...
@section('initialStore')
    ...
    window['{{ config('twill.js_namespace') }}'].STORE.datatable.data =
    window['{{ config('twill.js_namespace') }}'].STORE.datatable.all
