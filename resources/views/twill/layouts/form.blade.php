        ...
        <div class="navbar navbar--sticky" data-sticky-top="navbar">
            ...
            <a17-sticky-nav data-sticky-target="navbar" :items="{{ json_encode($additionalFieldsets) }}">
                <a17-title-editor name="{{ $titleFormKey }}" :editable-title="{{ json_encode($editableTitle ?? true) }}"
                    custom-title="{{ $customTitle ?? '' }}" slot="title"
...
@section('initialStore')
    ...
    window.STORE.publication = {
        ...
        submitOptions: @if (isset($item) && $item->cmsRestoring) {
            ...
            } window.STORE.revisions={!! json_encode($revisions ?? []) !!} window.STORE.parentId={{ $item->parent_id ?? 0 }}
            window.STORE.parents={!! json_encode($parents ?? []) !!} window.STORE.medias.crops={!! json_encode(($item->mediasParams ?? []) + config('twill.block_editor.crops')) !!}
            window.STORE.medias.selected={} window.STORE.browser={} window.STORE.browser.selected={}
            window.APIKEYS={ 'googleMapApi' : '{{ config('twill.google_maps_api_key') }}' } @stop @push('extra_js')
