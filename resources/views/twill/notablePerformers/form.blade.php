            ...
            <table class="table" id="songs-table">
                <tbody>
                    @foreach ($item->songs as $song)
                        <tr>
                            <td width="60"><img src="{{ $song->cmsImage() }}" alt="{{ $song->title }}" width="60" height="60" />
                            </td>
                            <td>{{ $song->title }}</td>
                            <td align="right"><a href="{{ route('twill.songs.edit', $song) }}">Edit</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
...
@include('twill/_includes/api-link', ['name' => 'notable-performers'])
@endsection
